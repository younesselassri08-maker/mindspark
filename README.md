...
1. In the `backend/.env` file, set `SMTP_EMAIL` to your Gmail address.
2. You **cannot** use your standard Gmail password. You must generate an App Password.
3. Go to your Google Account > **Security**.
4. Ensure **2-Step Verification** is turned on.
5. Search for **App passwords** in the security search bar.
6. Generate a new password (select "Mail" and "Other (Custom name)" like "MindSpark").
7. Copy the generated 16-character password (without spaces) and paste it as the `SMTP_PASSWORD` in your `.env` file.
...
2. In the frontend React code (specifically in the API service files), carefully replace any hardcoded `http://localhost:5001` URLs with `http://localhost:5000` to ensure proper Client-Server communication.
